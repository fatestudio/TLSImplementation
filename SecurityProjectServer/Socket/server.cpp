...
static char *data_to_client = "This is a server test!";
...
static int sendbuf_len;
static char recvbuf[DEFAULT_BUFLEN];
static int recvbuf_len = DEFAULT_BUFLEN;
...
bool outputUint64(uint64_t sequence_number, opaque *result){
	int i;
	for(i = 7; i >= 0; i--){
...
bool createNonce(){
	...
	nonce.write_IV = (client_write_IV[0]) + (client_write_IV[1] << 8) + (client_write_IV[2] << 16) + (client_write_IV[3] << 24);
...
bool createNonce2(){
	...
	nonce2.write_IV = (server_write_IV[0]) + (server_write_IV[1] << 8) + (server_write_IV[2] << 16) + (server_write_IV[3] << 24);
...
bool constructTLSPlaintext(){
	...
	plaintext.length = strlen(data_to_client);
	for(i = 0; i < plaintext.length; i++){
		plaintext.fragment[i] = data_to_client[i];
	}
	return true;
}
...
bool stringToTLSCiphertext(char *recvbuf, int recvbuf_len){
	...
	ciphertext2.fragment.nonce_explicit = recvbuf[5] + (recvbuf[6] << 8) + (recvbuf[7] << 16) + (recvbuf[8] << 24) + (recvbuf[9] << 32) + (recvbuf[10] << 40) + (recvbuf[11] << 48) + (recvbuf[12] << 56);
...
int __cdecl main(int argc, char **argv) 
{
	...
	// while loop for tls
	serverphase = InitialS;
	whileflag = true;
	while(whileflag){
		switch (serverphase) {
		case TestS: {
			strcpy(sendbuf, "Server Test");
			sendbuf_len = strlen(sendbuf);
			printf("%d\n%s\n", sendbuf_len, sendbuf);
			send(ClientSocket, sendbuf, sendbuf_len, 0 ); 
			
			Sleep(1000);

			recvbuf_len = recv(ClientSocket, recvbuf, recvbuf_len, 0);

			printf("Server Test Recv:\n");
			printf("%d\n", recvbuf_len);
		...
		case InitialS: {
			if(localtest && recordOnly){
				...
				serverphase = RecordPrepareS;
			}
			else if(recordOnly){
				// let server decide the security parameters, and send to 
				createSecurityParameters();
				securityParametersToString(sendbuf, &sendbuf_len); 
				send( ClientSocket, sendbuf, sendbuf_len, 0 ); 
				printf("Security Parameters Sent\n");
			...
			printf("InitialS Finished\n");
			break;
					  }
		case HandshakeWaitClientHello: {
			recvbuf_len = recv(ClientSocket, recvbuf, recvbuf_len, 0);
			parseClientHello(recvbuf, recvbuf_len);

			generateServerHello(sendbuf, &sendbuf_len);
			send(ClientSocket, sendbuf, sendbuf_len, 0);
			generateServerCertificate(sendbuf, &sendbuf_len);
			send(ClientSocket, sendbuf, sendbuf_len, 0);
			generateServerKeyExchange(sendbuf, &sendbuf_len);
			send(ClientSocket, sendbuf, sendbuf_len, 0);
			generateServerHelloDone(sendbuf, &sendbuf_len);
			serverphase = HandshakeWaitClientFinishedMessage;
			break;
									   }
		case HandshakeWaitClientFinishedMessage: {
			recvbuf_len = recv(ClientSocket, recvbuf, recvbuf_len, 0);
			parseClientKeyExchange(recvbuf, recvbuf_len);
			recvbuf_len = recv(ClientSocket, recvbuf, recvbuf_len, 0);
			parseClientCertificateVerify(recvbuf, recvbuf_len);
			recvbuf_len = recv(ClientSocket, recvbuf, recvbuf_len, 0);
			parseClientFinishedMessage(recvbuf, recvbuf_len);
			
			serverphase = RecordLayerS;
			break;
									   }
		case RecordPrepareS: {
			if(!localtest){
			...
			serverphase = RecordLayerS;
			printf("RecordPrepareS Finished\n");
			break;
							}
		case RecordLayerS: {
			if(localtest){
				...
				bool valid = AEAD_Decrypt(client_write_key, nonce2, compressedtext.fragment, &(compressedtext.length), additional_data, additional_data_len, ciphertext.fragment.content, ciphertext.length);
				...
				createNonce();
				createAdditionalData();
				AEAD_Encrypt(server_write_key, nonce, compressedtext.fragment, compressedtext.length, additional_data, additional_data_len, AEADEncrypted, &AEADEncrypted_len);
				constructTLSCiphertext(AEADEncrypted, AEADEncrypted_len);
				TLSCiphertextToString(sendbuf, &sendbuf_len);

				send(ClientSocket, sendbuf, sendbuf_len, 0);
				printf("Server RecordLayer Send Finished\n");

				recvbuf_len = recv(ClientSocket, recvbuf, recvbuf_len, 0);
				printf("Client Ciphertext Received:\t%d\n", recvbuf_len);
